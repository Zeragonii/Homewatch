...
function deviceTemplate(d){return `<div class="row"><div><h3 class="device-title"></h3><span class="device-status"></span></div><small class="muted device-agent"></small></div><div class="device-assignment"><label><b>Child:</b> <select class="device-child-select"></select></label><button class="secondary device-child-save" onclick="saveDeviceChild('${escAttr(d.id)}',this)">Save</button></div><p><b>User:</b> <span class="device-user"></span></p><p><b>Current:</b> <span class="device-current"></span></p><div class="activity"></div><div class="message-composer"><div class="message-composer-head"><h4>Message</h4><select class="message-type" aria-label="Message type"><option value="notify">Notify</option><option value="question">Question</option><option value="alert">Alert</option></select></div><div class="message-row"><input class="message-input" placeholder="Type a message…"><button onclick="sendMessage('${escAttr(d.id)}',this)">Send</button></div><small class="muted message-hint">Notify is dismissible. Question requires a reply. Alert requires acknowledgement.</small></div><div class="actions"><button class="secondary" onclick="shot('${escAttr(d.id)}')">Screenshot</button><button class="secondary update-btn" onclick="checkUpdate('${escAttr(d.id)}',this)">Update now</button></div><div class="device-controls"><h4>Device controls</h4><div class="control-actions"><button class="secondary" onclick="deviceCommand('${escAttr(d.id)}','lock')">Lock</button><button class="secondary" onclick="deviceCommand('${escAttr(d.id)}','logoff')">Log off</button><button class="danger" onclick="deviceCommand('${escAttr(d.id)}','restart')">Restart</button><button class="danger" onclick="deviceCommand('${escAttr(d.id)}','shutdown')">Shutdown</button></div><div class="close-app-row"><input class="close-app-input" placeholder="Process name, e.g. wow.exe"><button class="secondary" onclick="closeApp('${escAttr(d.id)}')">Close app</button></div></div><details class="history"><summary>Command & message history</summary><div class="command-history"></div></details><img class="screenshot hidden" alt="Latest screenshot">`}
function updateDeviceCard(card,d){card.querySelector('.device-title').textContent=`${d.child} — ${d.name}`;const status=card.querySelector('.device-status');status.className=`device-status ${d.online?'online':'offline'}`;status.textContent=`● ${d.online?'Online':'Offline'}`;card.querySelector('.device-agent').textContent=`Agent ${d.agent_version||'—'}`;card.querySelector('.device-user').textContent=d.logged_in_user||'Not reported';card.querySelector('.device-current').textContent=d.current_app||'No foreground app reported';card.querySelector('.activity').innerHTML=d.activity.map(a=>`<div><span>${esc(a.process)}</span><b>${fmt(a.seconds)}</b></div>`).join('')||'<span class="muted">No activity today.</span>';const select=card.querySelector('.device-child-select');if(select){const selected=select.value;const ids=state.children.map(c=>String(c.id));const existing=[...select.options].map(o=>o.value);if(JSON.stringify(existing)!==JSON.stringify(ids)){select.replaceChildren(...state.children.map(c=>{const o=document.createElement('option');o.value=c.id;o.textContent=c.name;return o}));}if(document.activeElement!==select||!selected)select.value=String(d.child_id);else if(ids.includes(selected))select.value=selected;}const close=card.querySelector('.close-app-input');if(close&&!close.value&&d.current_app)close.placeholder=`Process name (${d.current_app})`;renderCommandHistory(card,d.commands||[])}
...
async function saveDeviceChild(id,button){
  const card=devicesEl.querySelector(`[data-device-id="${cssEscape(id)}"]`);
  const select=card?.querySelector('.device-child-select');
  const childId=Number(select?.value);
  if(!childId)return;
  const current=state.devices.find(d=>d.id===id);
  if(current&&Number(current.child_id)===childId)return;
  const child=state.children.find(c=>Number(c.id)===childId);
  if(!confirm(`Move ${current?.name||'this device'} to ${child?.name||'the selected child'}?`))return;
  const original=button?.textContent||'Save';
  if(button){button.disabled=true;button.textContent='Saving…'}
  try{
    await api(`/api/devices/${id}/child`,{method:'PUT',body:JSON.stringify({child_id:childId})});
    await refresh();
    if(currentView==='screentime')await loadScreenTime();
    if(button)button.textContent='Saved';
    setTimeout(()=>{if(button){button.disabled=false;button.textContent=original}},1200);
  }catch(e){
    if(button){button.disabled=false;button.textContent=original}
    alert(`Could not reassign device: ${e.message}`);
  }
}

async function sendMessage(id,button){
  const card=devicesEl.querySelector(`[data-device-id="${cssEscape(id)}"]`),input=card?.querySelector('.message-input'),typeEl=card?.querySelector('.message-type');
  const text=input?.value.trim(),type=typeEl?.value||'notify';if(!text)return;
  const original=button?.textContent||'Send';if(button){button.disabled=true;button.textContent='Sending…'}
  try{
    await api(`/api/devices/${id}/commands`,{method:'POST',body:JSON.stringify({kind:'message',payload:JSON.stringify({type,text})})});
    input.value='';if(button)button.textContent='Sent';setTimeout(()=>{if(button){button.disabled=false;button.textContent=original}},1000);await refresh();
  }catch(e){if(button){button.disabled=false;button.textContent=original}alert(`Could not send message: ${e.message}`)}
}
...
function parseMessagePayload(payload){try{const p=JSON.parse(payload);if(p&&['notify','question','alert'].includes(p.type)&&typeof p.text==='string')return p}catch{}return{type:'notify',text:String(payload||'')}}
function messageTypeLabel(type){return({notify:'Notify',question:'Question',alert:'Alert'})[type]||'Notify'}
function renderCommandHistory(card,commands){
  const el=card.querySelector('.command-history');if(!el)return;
  el.innerHTML=commands.length?commands.map(c=>{
    if(c.kind==='message'){
      const m=parseMessagePayload(c.payload);return `<div class="command-row message-history-row message-${escAttr(m.type)}"><div><div><span class="message-type-badge">${esc(messageTypeLabel(m.type))}</span></div><div class="message-history-text">${esc(m.text)}</div></div><div><span class="command-status status-${escAttr(c.status)}">${esc(c.status)}</span>${c.result?`<span class="command-result message-response">${esc(c.result)}</span>`:''}</div></div>`;
    }
    return `<div class="command-row"><div><b>${esc(commandLabel(c.kind))}</b>${c.kind==='close_app'&&c.payload?` <span class="muted">${esc(c.payload)}</span>`:''}</div><div><span class="command-status status-${escAttr(c.status)}">${esc(c.status)}</span>${c.result?`<span class="muted command-result">${esc(c.result)}</span>`:''}</div></div>`;
  }).join(''):'<span class="muted">No commands yet.</span>';
}
